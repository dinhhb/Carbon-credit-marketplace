...
const PendingRetirementInfoModal: React.FC<RetirementProps> = ({
  retirement,
}) => {
  const [pendingRetirementInfoModalOpen, setPendingRetirementInfoModalOpen] =
    useState(false);
  ...
  useEffect(() => {
    const clickHandler = ({ target }: MouseEvent) => {
      ...
      if (
        !pendingRetirementInfoModalOpen ||
        modal.current.contains(target) ||
        trigger.current.contains(target)
      )
        return;
      setPendingRetirementInfoModalOpen(false);
    };
  ...
  useEffect(() => {
    const keyHandler = ({ keyCode }: KeyboardEvent) => {
      if (!pendingRetirementInfoModalOpen || keyCode !== 27) return;
      setPendingRetirementInfoModalOpen(false);
    };
  ...
  return (
    <div>
      <button
        ref={trigger}
        onClick={() =>
          setPendingRetirementInfoModalOpen(!pendingRetirementInfoModalOpen)
        }
      ...
      <div
        className={`fixed left-0 top-0 z-999999 flex h-full min-h-screen w-full items-center justify-center bg-black/90 px-4 py-5 ${
          pendingRetirementInfoModalOpen ? "block" : "hidden"
        }`}
      >
        <div
          ref={modal}
        ...
        >
          ...
          <div className="mb-4 flex items-center text-left">
            <p className="flex-1">VCU Retirement Certificate:</p>
            <p className="flex-1">
...
export default PendingRetirementInfoModal;
